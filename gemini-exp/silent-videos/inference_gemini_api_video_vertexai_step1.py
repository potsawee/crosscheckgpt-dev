...
def exp():
  ...
  paths = glob("silent_videos/*.mp4")
  random.shuffle(paths)


  for file_path in tqdm(paths):
    file_name = file_path.replace("silent_videos/", "").replace(".mp4", "")
    video_file_uri = (
        f"gs://crosscheck-videos/silent_videos/{file_name}.mp4"
    )
    video_file = Part.from_uri(video_file_uri, mime_type="video/mp4")


    output_path = f"outputs_step1/{file_name}.json"
