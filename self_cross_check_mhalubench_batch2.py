...
def main(
    ...
    # LLM to be evaluated
    with open(f"mhalubench/MoreOnebest/{llm_evaluated}_onebest.json", "r") as f:
        onebest = json.load(f)
    ...
    for evidence_llm in evidence_llms:
        ...
        output_path = f'outputs/mhalubench_batch2/{llm_evaluated}_{evidence_llm}.json'
        if os.path.isfile(output_path):
            print("{}: already exists".format(output_path))
            continue    

        outputs = []        
        with open(f"mhalubench/MorePassages/{evidence_llm}_passages.json", "r") as f:
